function doGet(e) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    totalCandidates++;
    if (colSource) {
      var source = colSource.toString().toUpperCase().trim();
      sourceWiseLeads[source] = (sourceWiseLeads[source] || 0) + 1;
      
      if (!sourceFunnel[source]) {
          sourceFunnel[source] = { count: 0, screening: 0, r1: 0, r2: 0 };
      }
      sourceFunnel[source].count++;
      if (colScreeningStatus && colScreeningStatus.trim() !== "") sourceFunnel[source].screening++;
      if (col1stRound && col1stRound.trim() !== "") sourceFunnel[source].r1++;
      if (col2ndRound && col2ndRound.trim() !== "") sourceFunnel[source].r2++;
    }
  ...
  var sourceFunnelRows = [];
  var sortedSources = Object.keys(sourceFunnel).sort();
  for (var k = 0; k < sortedSources.length; k++) {
    var src = sortedSources[k];
    sourceFunnelRows.push([
      src,
      sourceFunnel[src].count,
      sourceFunnel[src].screening,
      sourceFunnel[src].r1,
      sourceFunnel[src].r2
    ]);
  }
